...
class FindThePairsButton: SKNode {
  ...
  var isFolded = false
  var buttonIsLocked = false
  
  init(buttonImage: String, cardImage: String, title: String, withStars: Bool, starsCount: Int, buttonAction: @escaping () -> Void) {
  ...
  func foldBackCard() {
    if !buttonIsLocked {
      button.texture = SKTexture(imageNamed: self.buttonImage)
      isFolded = false
    }
  }
